...
function updateCharts() {
  ...
  // If no transactions, exit gracefully
  if (!txHistory.length) {
  ...
  // No need to filter by type strictly
  const sentTransactions = txHistory.filter(tx => (tx.type?.toLowerCase() || "sent") === "sent");
...
function processTransactionData(transactions) {
  ...
  transactions.forEach(tx => {
    if (!tx.date || typeof tx.date !== "string" || !tx.amount) return;
...
function parseTransactionDate(dateString) {
  try {
    ...
    return new Date(dateString); // Fallback for ISO formats
  } catch (e) {
    console.error("Date parsing error:", e);
    return new Date(NaN);
  }
  ...
  if (spendingChart) {
    ...
    spendingChart = new Chart(ctx, {
      ...
      options: {
        responsive: true,
        plugins: {
          title: { display: true, text: 'Spending Over Time', font: { size: 18 } },
          tooltip: {
            callbacks: {
              label: context => `${context.dataset.label}: ${context.parsed.y} XLM`
            }
  ...
  if (recipientChart) {
    ...
    recipientChart = new Chart(ctx, {
      ...
      options: {
        responsive: true,
        plugins: {
          title: { display: true, text: 'Top 5 Recipients', font: { size: 18 } },
          tooltip: {
            callbacks: {
              label: context => `${context.dataset.label}: ${context.parsed.y} XLM`
            }
