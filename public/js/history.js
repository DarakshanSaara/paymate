...
function applyFirestoreFilters() {
  ...
  db.collection("transactions")
    ...
    .then((querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        const tx = doc.data();
        // Ensure address is lowercased
        const addressLower = address ? address.toLowerCase() : '';
        
        // Check if the transaction matches the address
        const matchesAddress = addressLower
          ? (tx.sender && tx.sender.toLowerCase().includes(addressLower) || tx.receiver && tx.receiver.toLowerCase().includes(addressLower))
          : true;
    
        // Check if the transaction matches the amount
        const matchesAmount = amount ? tx.amount == amount : true;
    
        // Check if the transaction matches the type
        const matchesType = type ? tx.type === type : true;
    
        // If all conditions are met, push the transaction to filtered array
        if (matchesAddress && matchesAmount && matchesType) {
          filtered.push(tx);
        }
    });    
